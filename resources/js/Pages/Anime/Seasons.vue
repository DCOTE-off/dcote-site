...
<template>
    <Breadcrumbs :items="[{ text: 'ГЛАВНАЯ', href: route('home') }, { text: 'АНИМЕ' }]" />
        ...
        <div class="media-card__cover">
            <picture>
                <source
                    ...
                    :srcset="`/images/anime/anime-banner-season-${season.season_number}-mobile.webp`"
                    type="image/webp" />
                <img
        ...
        <div class="media-card__desc">
            <div class="media-card__head">
                ...
                <div
                    ...
                    <div class="extra-info">
                        <p>
                            Всего оценок:
                            <span class="ratings-count">{{ season.season_ratings_count }}</span>
                        </p>
                    </div>
                </div>
            </div>
            <Collapse :always-open="!isMobile" class="info-panel">
                ...
                <dl class="info-panel__grid">
                    ...
                    <dd class="info-panel__value">
                        {{ season.season_time === 'null' ? 'Не определено' : season.season_time }}
                    </dd>
                    <dt class="info-panel__label">День релиза:</dt>
                    <dd class="info-panel__value">
                        {{ season.release_time === 'null' ? 'Не определено' : season.release_time }}
                    </dd>
                    <dt class="info-panel__label">Студия:</dt>
                    <dd class="info-panel__value">
                        {{ season.studio === 'null' ? 'Не определено' : season.studio }}
                    </dd>
                    <dt class="info-panel__label">Кол-во серий:</dt>
                    <dd class="info-panel__value">
                        {{ season.number_of_episodes === 0 ? 'Не определено' : season.number_of_episodes }}
                    </dd>
                    <dt class="info-panel__label">
                        {{ isMobile ? 'Экранизация:' : 'Экранизируемые тома:' }}
                    </dt>
                    <dd class="info-panel__value">
                        {{ season.adapt_volumes === 'null' ? 'Не определено' : season.adapt_volumes }}
                        {{ season.adapt_volumes_brackets === 'null' ? '' : season.adapt_volumes_brackets }}
                    </dd>
                </dl>
            </Collapse>
            <div class="media-card__progress">
                <p>
                    <b>Выпущено:</b> {{ season.released_episodes_count }} из
                    {{ season.number_of_episodes === 0 ? '?' : season.number_of_episodes }} серий
                </p>
                <div
                    class="media-card__progress-bar"
                    :style="{ '--progress-width': `${seasonPercent(season)}%` }"></div>
            </div>
            <button v-if="season.is_announced" type="button" class="btn-pill" disabled>СТРАНИЦА СЕЗОНА</button>
            <Link v-else class="link-pill" :href="route('anime.season', { season: season.season_number })"
                >СТРАНИЦА СЕЗОНА</Link
            >
            <div class="media-card__buttons">
                ...
                <Link
                    ...
                    style="border-color: rgba(98, 59, 146, 1)"
                    :href="route('anime.episode', { season: season.season_number, episode: 1 })"
                    >НАЧАТЬ СМОТРЕТЬ</Link
                >
            </div>
