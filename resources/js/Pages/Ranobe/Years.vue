...
<template>
    ...
    <div class="year-grid">
        <div
            ...
            class="year-card">
            <img :src="`/images/ranobe/years/y${year.year_number}v1.webp`" class="year-card__cover" :alt="`Обложка ${year.year_readable}`">
            <div class="year-card__body">
                ...
                <Link class="link-pill year-card__link" :href="route('ranobe.year', { year: year.year_number })">ПЕРЕЙТИ</Link>
                <div class="year-card__status">
                    <svg class="year-card__status-icon">
                        <use :href="year.status === 'Завершён' ? '#check-circle' : '#clock-logo'" />
                    </svg>
                    <p class="year-card__status-text" :class="year.status === 'Завершён' ? 'year-card__status-text--completed' : 'year-card__status-text--ongoing'">
                        {{ year.status }}
                    </p>
                </div>
            </div>
